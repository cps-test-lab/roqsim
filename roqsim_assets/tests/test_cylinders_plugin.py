...
def test_physics_instances_get_a_base_joint_registered():
    ...
    _, _, ctx = _build(
        name="clutter",
        instances=[{"pos": [0.1, 0.0], "radius": 0.03, "height": 0.15, "motion": "physics"}],
    )
...
def test_a_free_instance_is_reseated_on_reset():
    ...
    model, plugin, ctx = _build(
        name="clutter",
        instances=[{"pos": [0.2, 0.1, 0.5], "radius": 0.03, "height": 0.15, "motion": "physics"}],
    )
...
def test_an_override_changes_the_population_count():
    ...
    twenty = [
        {"pos": [0.02 * i, 0.0], "radius": 0.03, "height": 0.15, "motion": "physics"} for i in range(20)
    ]
