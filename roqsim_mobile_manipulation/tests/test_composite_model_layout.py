...
Three failures this catches, none of which the per-robot drive tests would:
...
* a mesh (or licence) that no ``[tool.setuptools.package-data]`` glob matches. That installs cleanly
  from an editable checkout and fails at RUN time inside a container, where the checkout is not there
  to paper over it -- the reason those globs are patterns rather than an enumeration;
* a vendor mesh redistributed without the licence sidecar that permits it.

Named ``test_composite_model_layout`` rather than the siblings' ``test_model_layout`` /
``test_sensor_model_layout`` / ``test_layout``: pytest cannot import two same-named test modules in
one run, and the whole tree is collected at once by ``make test``.

A composite mobile manipulator borrows another package's meshes (Frankie takes the Panda's via the
manifest's ``assets:`` key), so ``test_model_resolves_with_all_meshes`` is the one test that proves
the borrow still resolves after a layout change -- a borrowed ref is written against the *provider's*
mesh root, not this package's.
"""
...
import mujoco
import pytest
from roqsim_mobile_manipulation.models import MODELS_DIR
...
def test_every_model_is_a_folder():
    ...
    assert set(MODEL_NAMES) == {"frankie", "tiago_pro"}


@pytest.mark.parametrize("name", MODEL_NAMES)
def test_model_resolves_with_all_meshes(name):
    asset = resolve_model(f"roqsim_mobile_manipulation:{name}")
    ...
    for mesh in spec.meshes:
        ...
        # found in any searched dir (this package's, or a provider named in the manifest's `assets:`).
...
def test_model_folder_ships_manifest_and_licence(name):
    """A model folder must be self-describing and redistributable on its own."""
...
def test_package_data_globs_cover_every_shipped_file():
    ...
    globs = data["tool"]["setuptools"]["package-data"]["roqsim_mobile_manipulation"]
    pkg = root / "src" / "roqsim_mobile_manipulation"
    missing = [
