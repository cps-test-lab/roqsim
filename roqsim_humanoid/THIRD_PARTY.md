...
| --- | --- |
| `models/oli.xml` | **Built** from `HU_D04_description/urdf/HU_D04_01.urdf` (the serial / PR-space tree) by `external/convert/build_oli.py`. Armatures from `urdf/HU_D04_01.srdf` (`rotor_mass × gear_ratio²`); collision primitives transplanted from `xml/HU_D04_01.xml`; base renamed to `base_link`/`base_free`, `lidar`/`imu` sites added, home keyframe baked (the head/chest D435s are `realsense_d435` devices the manifest mounts). The parallel ankle/waist linkages are approximated as serial joints. |
...
| --- | --- |
| `models/unitree_g1.xml` | `resources/robots/g1_description/g1_12dof.xml` — **adapted**: base body/joint renamed `pelvis`→`base_link`, `floating_base_joint`→`base_free`; the head mesh `head_link` swapped for `head_link_mid360_window.STL`, cut from the vendored decimated `head_link.STL` with the opening for the Livox Mid-360 by `external/convert/g1_head_window.py` (sensor pose from unitree_ros `g1_29dof_rev_1_0.urdf` @ f3772ce; its housing and field from the Livox Mid-360 User Manual v1.2); dropped the standalone `scene.xml` include (the roqsim world provides ground + lighting). Bodies, joints, inertials, geoms and `<motor>` actuators are otherwise verbatim. |
...
| --- | --- |
| `models/unitree_g1_dex1.xml` | **Built** by `external/convert/build_g1_dex1.py` from `robots/g1_description/g1_29dof_mode_15_with_dex1_1.urdf`. Built from the URDF and not from the sibling `g1_29dof_rev_1_0.xml` MJCF because the two disagree on the wrist (`wrist_yaw` at x=0.051 vs 0.046 — different `_5010` hardware). Adaptations: `pelvis`→`base_link`, `floating_base_joint`→`base_free`, rubber hands dropped (upstream carries them *and* the gripper on the same mount), finger range clamped to the non-crossed half, four-sphere foot contacts transplanted from `unitree_g1.xml`, static links kept as welded bodies (MuJoCo's `to_xml` of a fused URDF import drops the fused offsets and masses; the build checks every mesh pose, the total mass and the centre of mass against MuJoCo's direct compile), the head mesh cut with the Livox Mid-360's opening by `external/convert/g1_head_window.py`, position actuators + gripper tendon/equality added, upstream `<light>`/floor dropped. |
