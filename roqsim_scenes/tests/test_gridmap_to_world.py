...
def test_world_yaml_round_trips_and_places_the_robot_first(tmp_path):
    ...
    spawn_entry = loaded["components"][0]
    position = spawn_entry["spawn_robot"]["pose"]["position"]
    assert (position["x"], position["y"]) == pytest.approx((1.0, 2.0))
    # `name` is a sibling of the plugin ref, like every other generated entry. Nested inside
    # `spawn_robot` it is a stray config key rather than the entry's label.
    assert spawn_entry["name"] == "robot"
    assert "name" not in spawn_entry["spawn_robot"]
...
def test_the_generated_world_loads(tmp_path):
    ...
    whose output the simulator will not accept is broken however well-formed its YAML is.
    """
    grid = _grid(["#.", ".#"])
    world = g2w.build_world(
        grid,
    ...
    config = load_config(str(tmp_path / "w.yaml"))

    assert any(spec.label == "robot" for spec in config.plugins), \
        "the entry's label is what a capture plugin or a scenario addresses it by"
