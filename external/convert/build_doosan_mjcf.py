...
from urdf_source import (  # noqa: E402
    expand_xacro, inertial, link_visuals, pose, rpy_to_quat, write_license,
)
...
TEMPLATE = """<mujoco model="m1013">
  <!--
    ...
    dsr_description2 @ {commit} (Apache-2.0 - see M1013_LICENSE).
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        xml = build(expand_xacro(sources, sources["dsr_description2"] / f"xacro/{MODEL}.urdf.xacro",
                                     Path(tmp), args=[f"color:={COLOURWAY}", "gripper:=none"]), materials)
    write_license(
        sources["dsr_description2"].parent / "LICENSE",
        PKG / "M1013_LICENSE",
        [
            "Doosan Robotics M1013 -- vendored geometry and description.",
            "",
            f"Upstream:   {DOOSAN_URL.removesuffix('.git')}  (dsr_description2)",
            f"Commit:     {DOOSAN_COMMIT}",
            "Copyright:  Doosan Robotics (ros.robotics@doosan.com)",
            "License:    Apache License 2.0. This is the repository's own LICENSE file, and",
            "            dsr_description2/package.xml declares the same. GitHub's repository",
            "            metadata says BSD-3-Clause; the file and the manifest are authoritative.",
            "",
            "Regenerate with: external/convert/build_doosan_mjcf.py",
