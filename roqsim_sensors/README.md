...
| `rplidar_c1` | Slamtec RPLIDAR C1 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` (500 rays over a full turn, 0.05–12 m, 10 Hz), excluding only its own `mount`, stamped in `laser` and publishing Husarion's `rplidar_link` → `laser` chain. Mounted by `roqsim_mobile`'s `rosbot`. Mesh and link frames from Husarion `husarion_components_description` — see `models/rplidar_c1/rplidar_c1_LICENSE`. |
...
The six 2D scanners are device models a robot mounts from its manifest: a `spawn_sensor` at the
