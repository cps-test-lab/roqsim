...
- **Look direction.** A device mounted by its vendor link (`oakd_pro`: `oakd_link`, x out of the lens,
  z up) looks along its local **+x** at `rpy [0,0,0]`, because `pos`/`rpy` are the vendor joint
  origin wherever it hangs, and it publishes its vendor frame chain. The other standalone camera
  mounts look along their local **+y** (out the lens), **+z up**, at
...
| `sick_nanoscan3` | SICK nanoScan3 safety laser scanner: housing mesh + a `scan` site. Its manifest attaches `lidar` as SICK's `sick_safetyscanners2` driver publishes it (1651 rays over 275° with the last on +137.5°, 0.1–40 m, 33.3 Hz), excluding only its own `mount`, stamped in `lidar_1_link`. Mounted twice by `roqsim_mobile`'s `rox_diff`. Its vendor link frame is **z-down**, so it renders inverted unless mounted at the vendor joint origin — see the MJCF header. Mesh and link frame from Neobotix `rox`, whose repository declares its licence in the package manifest and ships no licence text — see `models/sick_nanoscan3/sick_nanoscan3_LICENSE`. |
| `sick_tim571` | SICK TiM571 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` as SICK's `sick_scan_xd` driver publishes it (811 rays over 270° with the last on +135°, header 0–100 m, returns measured 0.05–25 m, 15 Hz), excluding only its own `mount`; it has no default `frame_id`, so the mount names it. Mounted twice by `roqsim_mobile_manipulation`'s `tiago_pro`, which overrides the values PAL's configuration and `sick_tim` driver set differently. Mesh and link frame from PAL `pal_urdf_utils` — see `models/sick_tim571/sick_tim571_LICENSE`. |
| `rplidar_a1` | Slamtec RPLIDAR A1 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` as `rplidar_ros` publishes it on a TurtleBot 4 (1080 rays from π − 359° to π, 0.15–12 m, 5.5 Hz), excluding only its own `mount`, stamped in `rplidar_link`. Mounted by `roqsim_mobile`'s `turtlebot4`, which overrides it to the 360 rays of Clearpath's datasheet (1° angular resolution). Mesh and link frame from `turtlebot4_description` — see `models/rplidar_a1/rplidar_a1_LICENSE`. |
| `rplidar_c1` | Slamtec RPLIDAR C1 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` as `sllidar_ros2` publishes it (720 rays over −π..π with the last on π, 0.05–12 m, a too-close surface at its measured distance, 10 Hz), excluding only its own `mount`, stamped in `laser` and publishing Husarion's `rplidar_link` → `laser` chain. Mounted by `roqsim_mobile`'s `rosbot` and `lgdxrobot2`. Mesh and link frames from Husarion `husarion_components_description` — see `models/rplidar_c1/rplidar_c1_LICENSE`. |
| `rplidar_s3` | Slamtec RPLIDAR S3 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` as `sllidar_ros2` publishes it (3240 rays over −π..π with the last on π, 0.05–40 m, a too-close surface at its measured distance, 10 Hz), excluding only its own `mount`, stamped in `laser` 30.5 mm above the housing base, the frame Husarion names `<name>_laser`. Mounted by `roqsim_mobile`'s `panther`. Mesh and link frame from Husarion `husarion_components_description` — see `models/rplidar_s3/rplidar_s3_LICENSE`. |
| `lds01` | ROBOTIS LDS-01 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` as `hls_lfcd_lds_driver` publishes it (360 rays with the last on 2π − 1°, 0.12–3.5 m, range-dependent noise, 5 Hz), excluding only its own `mount`, stamped in `base_scan`. Mounted by `roqsim_mobile`'s `turtlebot3_waffle` and `raspimouse`. Mesh and link frame from ROBOTIS `turtlebot3_description` — see `models/lds01/lds01_LICENSE`. |
| `omron_os32c` | Omron OS32C safety laser scanner: a **primitive** housing (133.0 × 142.7 mm body, 100 mm sensor head, 104.5 mm overall, 1.3 kg) + a `scan` site 67 mm above its base, all from Omron's data sheet Z298-E2-05-X. Its manifest attaches `lidar` with the data sheet's 270° at 0.4°, 15 m warning zone and a derived 25 Hz, laid out as the ROS 1 `omron_os32c_driver` publishes it (677 rays over ±135.2°, header 0.002–50 m, no return 50.0, no noise figure in the data sheet), excluding only its own `mount`, stamped in `laser`. Mounted by `roqsim_mobile_manipulation`'s `frankie`, which overrides the field to the LD platform's 240°. **No mesh on purpose**: Omron's CAD is for personal reference only and the community model has no licence — see `models/omron_os32c/omron_os32c_LICENSE`. |
| `hokuyo_ust` | Hokuyo UST-10LX 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` as `urg_node` publishes it (1081 rays over 270° with the last on +135°, header 0.06–10 m, returns measured 0.021–30 m, the device's error codes 0.004 m for too close and 65.533 m for no return, 40 Hz), excluding only its own `mount`, stamped in `lidar2d_0_laser` 47.4 mm above the bracket base. Mounted by `roqsim_mobile`'s `ridgeback`, `husky_a200` and `warthog` (two, each limited to ±90°). Mesh and link frames from Clearpath `clearpath_sensors_description` — see `models/hokuyo_ust/hokuyo_ust_LICENSE`. |
...
The twelve scanners and the OAK-D Pro are device models a robot mounts from its manifest: a
`spawn_sensor` at the vendor's parent frame and joint origin, overriding a value only where its
vendor configuration differs.
...
|---------|-----|--------------|
| `oakd` | `oakd_pro` | The mount frame became the vendor `oakd_link`. It was a display convention: the body was turned +90° about z so a mount at `rpy [0,0,0]` looked along +y. |
...
renames the model and rewrites each mount's `pos`/`rpy` to `T_old * Rq`, which puts the camera where
it was. `tests/test_oakd_pro_device.py` holds that check.

## Demo world

`worlds/all_sensors_demo.yaml` places the camera and 3D-lidar mounts in the default empty room —
the `mid360`, `d435`, `d415`, `d455`, `zivid` and `oakd_pro` mounts (bringing `livox_mid360`,
...
every camera mount a synthesised frustum — including the Zivid, whose bundled envelope stays hidden
because a model with a camera always takes the frustum path. A
...
- The engine (`coverage/engine.py`) gates each point by range → angular FOV → line-of-sight
  (`raycast.cast`, masking group 4 so an absent entity cannot occlude; an FOV-visualisation mesh
