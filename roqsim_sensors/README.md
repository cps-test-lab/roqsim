...
| `sick_s300` | SICK S300 safety laser scanner: housing mesh + a `scan` site. Its manifest attaches `lidar` with the data sheet's values (540 rays over 270°, 0.1–30 m, 25 Hz), excluding only its own `mount`, stamped in `lidar_1_link` (the Neobotix link name). Mounted by `roqsim_mobile`'s `mp_400` (one) and `mpo_700` (two). Mesh and link frame from Neobotix `neo_simulation2` — see `models/sick_s300/sick_s300_LICENSE`. |
| `sick_microscan3` | SICK microScan3 Core safety laser scanner: housing mesh + a `scan` site. Its manifest attaches `lidar` (715 rays over 275°, 0.1–40 m, 25 Hz), excluding only its own `mount`, stamped in `lidar_1_link`. Mounted twice by `roqsim_mobile`'s `mpo_500`. Mesh and link frame from Neobotix `neo_simulation2` — see `models/sick_microscan3/sick_microscan3_LICENSE`. |
| `sick_tim571` | SICK TiM571 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` (818 rays over 270°, 0.05–25 m, 15 Hz), excluding only its own `mount`; it has no default `frame_id`, so the mount names it. Mounted twice by `roqsim_mobile_manipulation`'s `tiago_pro`, which overrides the values PAL configures differently. Mesh and link frame from PAL `pal_urdf_utils` — see `models/sick_tim571/sick_tim571_LICENSE`. |
| `rplidar_a1` | Slamtec RPLIDAR A1 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` (360 rays over a full turn, 0.15–12 m, 5.5 Hz), excluding only its own `mount`, stamped in `rplidar_link`. Mounted by `roqsim_mobile`'s `turtlebot4`. Mesh and link frame from `turtlebot4_description` — see `models/rplidar_a1/rplidar_a1_LICENSE`. |
| `rplidar_c1` | Slamtec RPLIDAR C1 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` (500 rays over a full turn, 0.05–12 m, 10 Hz), excluding only its own `mount`, stamped in `laser` and publishing Husarion's `rplidar_link` → `laser` chain. Mounted by `roqsim_mobile`'s `rosbot` and `lgdxrobot2`. Mesh and link frames from Husarion `husarion_components_description` — see `models/rplidar_c1/rplidar_c1_LICENSE`. |
| `lds01` | ROBOTIS LDS-01 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` (360 rays, 0.12–3.5 m, 5 Hz), excluding only its own `mount`, stamped in `base_scan`. Mounted by `roqsim_mobile`'s `turtlebot3_waffle` and `raspimouse`. Mesh and link frame from ROBOTIS `turtlebot3_description` — see `models/lds01/lds01_LICENSE`. |
| `hokuyo_ust` | Hokuyo UST-10LX 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` with the specification's values (1080 rays over 270°, 0.06–30 m, 40 Hz), excluding only its own `mount`, stamped in `lidar2d_0_laser` 47.4 mm above the bracket base. Mounted by `roqsim_mobile`'s `ridgeback`. Mesh and link frames from Clearpath `clearpath_sensors_description` — see `models/hokuyo_ust/hokuyo_ust_LICENSE`. |
| `sick_lms1xx` | SICK LMS111 2D lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` with the data sheet's values (540 rays over 270°, 0.5–20 m, 50 Hz), excluding only its own `mount`, stamped in `lidar2d_0_laser`. Clearpath's `sick_lms1xx` accessory. Mesh and link frames from Clearpath `clearpath_sensors_description` — see `models/sick_lms1xx/sick_lms1xx_LICENSE`. |
| `velodyne_vlp16` | Velodyne VLP-16 (Puck) lidar: housing mesh + a `scan` site. Its manifest attaches `lidar` casting the one horizontal plane of its 16 (1800 rays over a full turn, 0.9–100 m, 10 Hz), the planar projection a 2D costmap reads, excluding only its own `mount`, stamped in `velodyne` 37.7 mm above the housing base. Mounted by `roqsim_mobile`'s `clearpath_jackal`. Meshes and link frames from Dataspeed `velodyne_description` — see `models/velodyne_vlp16/velodyne_vlp16_LICENSE`. |

The nine scanners are device models a robot mounts from its manifest: a `spawn_sensor` at the
