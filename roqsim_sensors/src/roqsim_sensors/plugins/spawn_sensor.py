...
``motion: static`` is the default and what this plugin always did: the mount is welded into the
model. A welded body has neither a mocap slot nor a joint, so nothing can place it at all, and a
``set_entity_state`` naming it is refused rather than silently ignored.
...
class SpawnSensorPlugin(Plugin):
    ...
    def __init__(self, config=None, *, name=None, entity=None, label=None):
        ...
        # for a prop. `static` is the default and what this plugin always did: the mount is part
        # of the model, and nothing can move it.
