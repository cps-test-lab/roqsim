...
def test_a_low_friction_pair_makes_the_block_slide_further(tmp_path):
    ...
    slippery = _slide(_engine(tmp_path, {"geoms": ["slider_g", "floor"], "friction": 0.02}))
...
def test_a_pair_leaves_every_other_contact_alone(tmp_path):
    ...
    engine = _engine(tmp_path, {"geoms": ["slider_g", "floor"], "friction": 0.02})
...
def test_two_pairs_sharing_one_object_get_different_frictions(tmp_path):
    ...
    engine = _engine(
        tmp_path,
        {"geoms": ["slider_g", "floor"], "friction": 0.02},
        {"geoms": ["slider_g", "other_g"], "friction": 0.9},
    )
    engine.setup()
    m = engine.ctx.model
    got = {}
        ...
        load_config_from_dict(
            {
                "sim": {"pacing": "asap", "world": str(world)},
                "components": [
                    {"contact_pair": {"bodies": ["slider", "other"], "friction": 0.4}},
                ],
            },
            base_dir=tmp_path,
        )
...
@pytest.mark.parametrize(
    "cfg, message",
    [
        ({"friction": 0.3}, "exactly one of"),
        ({"geoms": ["a"], "friction": 0.3}, "exactly two names"),
        ({"geoms": ["a", "b"], "entities": ["x", "y"]}, "exactly one of"),
        ({"geoms": ["a", "b"], "friction": [1.0, 2.0]}, "5-element row"),
        ({"geoms": ["a", "b"], "friction": -1.0}, ">= 0"),
        ({"geoms": ["a", "b"], "condim": 2}, "condim"),
    ],
)
...
def test_declaring_the_same_pair_twice_is_refused(tmp_path):
    ...
    engine = _engine(
        tmp_path,
        {"geoms": ["slider_g", "floor"], "friction": 0.02},
        {"geoms": ["floor", "slider_g"], "friction": 0.9},
    )
...
def test_an_unknown_body_fails_loudly(tmp_path):
    ...
    engine = _engine(tmp_path, {"bodies": ["slider", "nope"], "friction": 0.3})
