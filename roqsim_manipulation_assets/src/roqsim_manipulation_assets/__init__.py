...
The ASSET half of the arm stack: the plugins (``spawn_arm``, ``arm_controller``, ...) live in
`roqsim_manipulation`, and the geometry -- 115 MB of it -- lives here.
...
it for Frankie's and TIAGo Pro's. In one package, each of those would drag in every arm and gripper
it never loads. The name follows ``roqsim_assets``, which is the same
